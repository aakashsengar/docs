...
<ParamField path="credentials" type="CredentialVault">
  Secure credential store. The LLM only sees placeholders like `{{email}}` — real values are injected at execution time and scrubbed from all output.
</ParamField>
...
| Example | Description |
|---------|-------------|
| `examples/30-browser-agent.ts` | Standalone browser agent — Hacker News search |
| `examples/31-browser-as-tool.ts` | Browser as a tool inside a research agent |
