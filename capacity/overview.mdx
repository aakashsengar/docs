...
During autoregressive generation, each transformer layer computes Key and Value projections for every token. Without caching, generating token N would require recomputing K and V for all N-1 prior tokens — O(N²) cost per step. The KV cache stores these projections so each decode step only reads them — reducing cost to O(N) per step.
...
| **GQA** | Grouped Query Attention | Multiple query heads share one KV head. Groups of query heads attend to the same K/V vectors. | < query heads | Reduced by group factor (typically 4-8×) |
...
The intermediate size of the feed-forward network inside each transformer layer. Typically 3-4× the hidden dimension. Affects prefill compute cost because FFN operations scale as O(N × ffn_dim) per layer.
...
Prefill is **compute-bound** (not memory-bound like decode) because attention is O(N²) in the prompt length.
...
Where efficiency is ~35% (real-world vs peak TFLOPS). The N² term (attention) dominates at long contexts — a 32K prompt takes ~64× longer than a 4K prompt, not 8×.
...
|----------|------------|------------------|---------------------|
| **Light** | 0 – 50K | Quick Q&A, lookups | < 7.8 GB |
...
**Why compute-bound:** Prefill runs the full attention computation (O(N²) in prompt length) plus FFN (O(N)). The GPU compute units are saturated, not memory.
...
The `4 × N²` attention term dominates. When N grows 8×, N² grows 64×. This is why long-context prefill is so expensive.
...
At 41 concurrent users, the average TTFT hits 5 seconds. The 42nd user will experience > 5s wait.
