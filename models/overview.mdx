...
RadarOS is **model-agnostic**. You can use OpenAI, Anthropic, Google Gemini, Ollama, or any custom provider through a unified `ModelProvider` interface. Switch models with a single line change—no refactoring required.
...
|---------|----------|--------|
| `openai(modelId, config?)` | OpenAI | `{ apiKey?, baseURL? }` |
| `anthropic(modelId, config?)` | Anthropic | `{ apiKey? }` |
| `google(modelId, config?)` | Google Gemini | `{ apiKey? }` |
| `ollama(modelId, config?)` | Ollama (local) | `{ host? }` (default: `http://localhost:11434`) |
