...
<ParamField body="reasoning" type="ReasoningConfig">
  Enable extended thinking / chain-of-thought reasoning. See [Reasoning](/agents/reasoning).
</ParamField>

<ParamField body="userMemory" type="UserMemory">
  User memory instance for cross-session personalization. See [User Memory](/agents/user-memory).
</ParamField>
...
<ParamField body="retry" type="Partial<RetryConfig>">
  Retry configuration for transient LLM API failures (429, 5xx, network errors). See [RetryConfig](#retryconfig).
</ParamField>

<ParamField body="maxContextTokens" type="number">
...
The `providerMetrics` field contains the raw, unmodified usage object returned by the underlying provider API. This varies by provider — see [Cost Tracking](/cost/overview#raw-provider-metrics) for examples per provider.
...
See [Tool Caching](/agents/tool-caching) for details.
...
<ParamField body="storage" type="StorageDriver">Storage backend for facts. Defaults to `InMemoryStorage`.</ParamField>
<ParamField body="model" type="ModelProvider">LLM for auto-extraction. Falls back to the agent's model.</ParamField>
<ParamField body="maxFacts" type="number" default="100">Maximum facts per user.</ParamField>
<ParamField body="enabled" type="boolean" default="true">Enable/disable auto-extraction.</ParamField>
...
See [User Memory](/agents/user-memory) for usage guide.
