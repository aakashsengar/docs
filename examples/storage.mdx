...
```typescript
import { Agent, openai, DynamoDBStorage } from "@radaros/core";
...
const storage = new DynamoDBStorage({
  ...
  tableName: "radaros-data",
});
await storage.initialize(); // Creates the table if it doesn't exist
...
const result = await agent.run("I'm a data scientist working on NLP", {
  sessionId: "s1",
  userId: "u1",
});
console.log(result.text);
...
// DynamoDB CRUD
await storage.set("my-namespace", "key-1", { data: "hello" });
const value = await storage.get("my-namespace", "key-1");
const items = await storage.list("my-namespace");
await storage.delete("my-namespace", "key-1");
...
```typescript
import { Agent, openai, MySQLStorage } from "@radaros/core";
...
await storage.initialize(); // Creates the kv table if needed
...
const result = await agent.run("Tell me about quantum computing", {
  sessionId: "session-1",
  userId: "user-1",
});
console.log(result.text);
...
```typescript
import { Agent, openai, RedisStorage } from "@radaros/core";
...
// Redis is great for short-lived, high-frequency sessions
for (let i = 0; i < 5; i++) {
  const result = await agent.run(`Message ${i + 1}: Tell me a fact`, {
    sessionId: "fast-session",
  });
  console.log(`Turn ${i + 1}: ${result.text.slice(0, 80)}...`);
}
